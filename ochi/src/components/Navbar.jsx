...
const Navbar = () => {
   ...
   const handleScroll = () => {
    ...
    if(currScrollY > scrollY) setVisible(false);
  ...
  return (
    <div className={`fixed w-full px-16 py-5 font-["Neue Montreal Regular"] flex justify-between items-center text-black backdrop-blur-sm z-20 transition-transform ${visible ? 'transform-none' : '-translate-y-full'} ease-in-out`}>
        <div className='logo'>
        ...
        <div className='links flex gap-10'>
            {
                ["Services", "Our work", "about Us", "insights", "Contact us"].map((item, index) => (
                    <a key={index} href="#" className={`text-md font-medium capitalize ${index === 4 && 'ml-[250px]'}`}>
                        <span className='flex flex-col relative overflow-hidden'>
                            <span className='hover:-translate-y-[125%]'>{item}</span>
                            <span className='absolute top-0 left-0'>{item}</span>
                        </span>
                    </a>
                ))
            }
