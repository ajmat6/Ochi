...
const Loader = () => {
  ...
  return (
    <div className='h-screen w-full bg-zinc-800 flex justify-center items-center'>
        <div className={`${load < 100 ? 'w-[80vw] h-[80vh]' : 'w-full h-full'} bg-zinc-100 text-black m-auto px-16 py-5 flex flex-col justify-between rounded-lg`}>
            <div className='logo'>
                    ...
                    ["We Create", "Eye-Opening", "Presentations"].map((item, index) => (
                        <div key={index} className='masker'>
                            <div className='w-fit flex'>
                                <h1 className='uppercase text-8xl font-["Test Founders Grotesk"] leading-[5.8vw] tracking-tighter font-medium'>{item}</h1>
                            </div>
                        </div>
                    ))
                }
            ...
            <div className='flex justify-between'>
                ...
                <div>
                    <div className='text-6xl font-bold'>{load}%</div>
                </div>
            </div>
        </div>
    </div>
